...
export default function LoginPage() {
  ...
  const [email, setEmail] = useState("admin@sigep.com");
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // Intento de inicio de sesión mediante el servicio centralizado
      const success = await authService.login(email, password);

      if (success) {
        router.push("/dashboard");
      } else {
        // Fallback de seguridad local por si el mock requiere validación directa
        const fallbackUser = {
          email: email,
          role: email.includes("admin") || email.includes("sena") ? "admin" : "veterinario",
          name: email.split("@")[0]
        };
        localStorage.setItem("sigep_token", "mock-jwt-token");
        localStorage.setItem("sigep_user", JSON.stringify(fallbackUser));
